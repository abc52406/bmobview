...
var configPlugins = [
    ...
    new webpack.optimize.CommonsChunkPlugin({
        ...
        filename: dirVars.buildName +'/[name].bundle.[chunkhash].js',
        minChunks: 4,
    }),
    /* 抽取出webpack的runtime代码()，避免稍微修改一下入口文件就会改动commonChunk，导致原本有效的浏览器缓存失效 */
    new webpack.optimize.CommonsChunkPlugin({
        name: 'webpack-runtime',
        filename: dirVars.buildName +'/webpack-runtime.[hash].js',
    }),
    /* 抽取出chunk的css */
    new ExtractTextPlugin(dirVars.buildName +'/[name].[contenthash].css'),
    /* 配置好Dll */
    ...
    new HtmlWebpackPlugin({
        filename: path.resolve(dirVars.staticRootDir, `./index.html`), 
        template: path.resolve(dirVars.srcRootDir, `./index.html`),
        chunks: ['webpack-runtime', 'index', 'commons'],
        hash: true, // 为静态资源生成hash值
        xhtml: true,
    }),
    //new CopyWebpackPlugin([{ from: path.resolve(dirVars.srcRootDir, `./dll/dll.js`), to: '../dll/' },],
